...
static PyObject* pyopencv_KAZE_KAZE(PyObject* , PyObject* args, PyObject* kw)
{
    pyopencv_KAZE_t* self = 0;

    if(PyObject_Size(args) == 0 && (kw == NULL || PyObject_Size(kw) == 0))
    {
        self = PyObject_NEW(pyopencv_KAZE_t, &pyopencv_KAZE_Type);
        new (&(self->v)) Ptr<cv::KAZE>(); // init Ptr with placement new
        if(self) ERRWRAP2(self->v = new cv::KAZE());
        return (PyObject*)self;
    }
...
static PyObject* pyopencv_SCascade_SCascade(PyObject* , PyObject* args, PyObject* kw)
{
    ...
    if( PyArg_ParseTupleAndKeywords(args, kw, "|ddii:SCascade", (char**)keywords, &minScale, &maxScale, &scales, &rejCriteria) )
    {
        self = PyObject_NEW(pyopencv_SCascade_t, &pyopencv_SCascade_Type);
        new (&(self->v)) Ptr<cv::SCascade>(); // init Ptr with placement new
        if(self) ERRWRAP2(self->v = new cv::SCascade(minScale, maxScale, scales, rejCriteria));
        return (PyObject*)self;
    }
